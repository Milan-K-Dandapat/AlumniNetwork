...
// --- ADMIN & VERIFICATION MIDDLEWARE DEFINITION ---
...
const isUserVerifiedMiddleware = (req, res, next) => {
    ...
    // 2. CRITICAL CHECK: Check the isVerified status attached by the 'auth' middleware (from JWT payload)
    if (req.user.isVerified !== true) { 
        console.log(`Access Denied to Career Page data for user ID: ${req.user._id}. Not verified.`);
        return res.status(403).json({ 
            message: `Access Denied. Your account is not yet verified by the Super Admin (${SUPER_ADMIN_EMAIL}).` 
        });
    }

    // 3. User is authenticated AND verified, proceed
    next();
};
// ⬆️ END USER VERIFICATION CHECK MIDDLEWARE ⬆️
...
// ⬇️ APPLY VERIFICATION MIDDLEWARE TO CAREER ROUTES ⬇️
app.use('/api/career-profile', auth, isUserVerifiedMiddleware, careerProfileRoutes);
app.use('/api/jobs', auth, isUserVerifiedMiddleware, jobRoutes); 
// ⬆️ END CAREER ROUTE PROTECTION ⬆️
