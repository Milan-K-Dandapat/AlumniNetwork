...
export const loginOtpSend = async (req, res) => {
    ...
    try {
        ...
        // ✅ FIX: Use findOneAndUpdate to set only OTP fields without full validation.
        let user = await Alumni.findOneAndUpdate(
            { $or: [{ email: identifier }, { phoneNumber: identifier }], isVerified: true },
            { $set: { otp, otpExpires } },
            { new: true }
        );
        ...
        const deliveryMethod = user.email ? 'email' : 'phone number';
...
export const loginOtpVerify = async (req, res) => {
    ...
    try {
        ...
        await user.save({ validateBeforeSave: false }); // Bypass validation for older users
